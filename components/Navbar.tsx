    ...
    <>
      <style>{`
        @keyframes dropAndSwing {
          0% { transform: perspective(1000px) rotateX(-90deg); animation-timing-function: cubic-bezier(0.25, 0.46, 0.45, 0.94); }
          50% { transform: perspective(1000px) rotateX(15deg); animation-timing-function: cubic-bezier(0.25, 0.46, 0.45, 0.94); }
      ...
      {/* ─── HEADER ─── */}
      <header className={`fixed top-0 left-0 w-full z-[999] transition-all duration-500 ease-in-out flex items-center justify-between ${isScrolled
        ...
        {/* ─── DESKTOP NAV ─── */}
        <nav
          className={`pointer-events-auto transition-all duration-500 ease-in-out hidden lg:flex items-center relative ${isScrolled ? 'px-0 py-0' : 'px-8 py-3.5'
        ...
        >
          ...
          <div className={`absolute inset-0 rounded-lg shadow-[0_15px_30px_-10px_rgba(0,0,0,0.3)] border-t border-white/20 border-b-2 border-[#8c5430]/30 transition-all duration-500 ease-in-out ${isScrolled ? 'opacity-0 scale-95' : 'opacity-100 scale-100'
            }`}>
      ...
      {/* ─── MOBILE DRAWER ─── */}
      <div className={`fixed top-0 right-0 z-[999] h-full w-[300px] max-w-[85vw] lg:hidden transition-transform duration-400 ease-[cubic-bezier(0.25,1,0.5,1)] ${mobileOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
          ...
          <div className="flex flex-col gap-1">
            {navLinks.map((item, i) => (
              <Link
                ...
                href={item.href}
                onClick={() => setMobileOpen(false)}
                className={`flex items-center gap-4 py-3.5 px-4 rounded-xl transition-all duration-300 group ${pathname === item.href
                    ? 'bg-[#8c5430]/20 text-[#fce8d5]'
                    : 'text-white/70 hover:bg-white/5 hover:text-white'
                  }`}
                style={{ animation: mobileOpen ? `mobileFadeIn 0.4s ease-out ${0.1 + i * 0.05}s both` : 'none' }}
              >
                <span className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${pathname === item.href ? 'bg-[#8c5430] scale-100' : 'bg-white/20 scale-75 group-hover:scale-100 group-hover:bg-white/40'
                  }`} />
